...
public class WorldModelB extends GridWorldModel {
    ...
    boolean move(Move dir, int ag) throws Exception {
        ...
        if (canMoveTo(nextX, nextY, ag)) {
            setAgPos(ag, nextX, nextY);
            restoreAgentMarker(l.x, l.y);
        }
        return true;
    }

    public boolean canMoveTo(int x, int y, int ag) {
        if (!inGrid(x, y) || hasObject(WorldModelB.OBSTACLE, x, y)) return false;
        return getOreAt(x, y) == 0 || !hasOtherAgentAt(x, y, ag);
    }

    private boolean hasOtherAgentAt(int x, int y, int ag) {
        for (int i = 0; i < nbAgs; i++) {
            if (i == ag) continue;
            Location pos = getAgPos(i);
            if (pos != null && pos.x == x && pos.y == y) return true;
        }
        return false;
    }

    private void restoreAgentMarker(int x, int y) {
        if (hasOtherAgentAt(x, y, -1)) {
            add(GridWorldModel.AGENT, x, y);
        }
    }
    ...
    */
    ...
    static WorldModelB world6() throws Exception {
        WorldModelB model = WorldModelB.create(35, 35, 9);
        model.setId("Scenario 6");
        model.setDepot(16, 16);
        model.setAgPos(0, 1, 0);
        model.setAgPos(1, 2, 0);
        model.setAgPos(2, 3, 0);
        model.setAgPos(3, 20, 0);
        model.setAgPos(4, 21, 0);
        model.setAgPos(5, 22, 0);
        model.setAgPos(6, 6, 26);
        model.setAgPos(7, 7, 26);
        model.setAgPos(8, 8, 26);

        model.add(WorldModelB.COAL, 6, 6);
        model.add(WorldModelB.COAL, 3, 5);
        model.add(WorldModelB.COAL, 2, 12);
        model.add(WorldModelB.COAL, 6, 9);

        model.add(WorldModelB.IRON, 20, 13);
        model.add(WorldModelB.IRON, 15, 20);
        model.add(WorldModelB.IRON, 2, 22);
        model.add(WorldModelB.IRON, 15, 12);

        model.add(WorldModelB.GOLD, 24, 24);
        model.add(WorldModelB.GOLD, 20, 21);
        model.add(WorldModelB.GOLD, 19, 2);
        model.add(WorldModelB.GOLD, 20, 15);

        model.add(WorldModelB.DIAMOND, 14, 4);
        model.add(WorldModelB.DIAMOND, 5, 17);
        model.add(WorldModelB.DIAMOND, 19, 29);
        model.add(WorldModelB.DIAMOND, 30, 7);

        model.add(WorldModelB.COAL, 5, 4);
        model.add(WorldModelB.COAL, 10, 10);
        model.add(WorldModelB.COAL, 11, 24);
        model.add(WorldModelB.COAL, 1, 30);

        model.add(WorldModelB.IRON, 22, 22);
        model.add(WorldModelB.IRON, 30, 30);
        model.add(WorldModelB.IRON, 11, 11);
        model.add(WorldModelB.IRON, 23, 3);

        model.add(WorldModelB.GOLD, 25, 18);
        model.add(WorldModelB.GOLD, 31, 31);
        model.add(WorldModelB.GOLD, 5, 21);
        model.add(WorldModelB.GOLD, 17, 10);

        model.add(WorldModelB.DIAMOND, 9, 27);
        model.add(WorldModelB.DIAMOND, 21, 14);
        model.add(WorldModelB.DIAMOND, 25, 29);
        model.add(WorldModelB.DIAMOND, 22, 30);
