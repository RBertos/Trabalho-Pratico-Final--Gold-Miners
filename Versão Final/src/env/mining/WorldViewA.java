...
public class WorldViewA extends GridWorldView {

    MiningPlanetA env = null;

    public WorldViewA(WorldModelA model) {
        super(model, "Mining World", 600);
        setVisible(true);
        repaint();
    }

    public void setEnv(MiningPlanetA env) {
        this.env = env;
        ...
        getCanvas().addMouseListener(new MouseListener() {
            public void mouseClicked(MouseEvent e) {
                ...
                if (col >= 0 && lin >= 0 && col < getModel().getWidth() && lin < getModel().getHeight()) {
                    WorldModelA wm = (WorldModelA)model;
                    wm.add(WorldModelA.GOLD, col, lin);
    ...
    public void draw(Graphics g, int x, int y, int object) {
        switch (object) {
        case WorldModelA.DEPOT:   drawDepot(g, x, y);  break;
        case WorldModelA.COAL:    drawOre(g, x, y, Color.darkGray); break;
        case WorldModelA.IRON:    drawOre(g, x, y, Color.lightGray); break;
        case WorldModelA.GOLD:    drawOre(g, x, y, Color.yellow); break;
        case WorldModelA.DIAMOND: drawOre(g, x, y, Color.cyan); break;
        case WorldModelA.ENEMY:   drawEnemy(g, x, y);  break;
        }
    }
    ...
    public void drawAgent(Graphics g, int x, int y, Color c, int id) {
        ...
        int carriedOre = wm.getCarriedOre(id);
        if (carriedOre != 0) {
            super.drawAgent(g, x, y, oreColor(carriedOre), -1);
        } else {
    ...
    private Color oreColor(int ore) {
        switch (ore) {
        case WorldModelA.COAL:    return Color.darkGray;
        case WorldModelA.IRON:    return Color.lightGray;
        case WorldModelA.GOLD:    return Color.yellow;
        case WorldModelA.DIAMOND: return Color.cyan;
