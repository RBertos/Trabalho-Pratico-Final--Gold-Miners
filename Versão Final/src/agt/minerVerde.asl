...
+cell(X,Y,Type,Value)
  :  role(explorer) & miner_id(Id) & Type \== obstacle & Value >= 3 & not scouted_ore(Type,X,Y,Value)
  <- +scouted_ore(Type,X,Y,Value);
     broadcastOre(Id,Type,X,Y,Value);
     .print("Green scout report: valuable ",ore(Type,X,Y,Value)).
+cell(X,Y,Type,Value)
  :  role(explorer) & Type \== obstacle & Value < 3 & not scouted_ore(Type,X,Y,Value)
  <- +scouted_ore(Type,X,Y,Value);
     .print("Green explorer ignored low value ore: ",ore(Type,X,Y,Value)).
+cell(X,Y,Type,Value)
  :  role(retriever1) & miner_id(Id) & Type \== obstacle & Value >= 3 & not ore(Type,X,Y,Value)
  <- +ore(Type,X,Y,Value);
     broadcastOre(Id,Type,X,Y,Value).
+cell(X,Y,Type,Value)
  :  role(retriever2) & miner_id(Id) & Type \== obstacle & not ore(Type,X,Y,Value)
  <- +ore(Type,X,Y,Value).

+radio_ore_0(Sender,Type,X,Y,Value,MsgId)
  :  .my_name(miner1) & role(retriever1) & Value >= 3 & Sender >= 0 & not ore(Type,X,Y,Value)
  <- +ore(Type,X,Y,Value);
     .print("Radio received from miner ",Sender+1,": ",ore(Type,X,Y,Value)).
+radio_ore_1(Sender,Type,X,Y,Value,MsgId)
  :  .my_name(miner2) & role(retriever1) & Value >= 3 & Sender >= 0 & not ore(Type,X,Y,Value)
  <- +ore(Type,X,Y,Value);
     .print("Radio received from miner ",Sender+1,": ",ore(Type,X,Y,Value)).
+radio_ore_2(Sender,Type,X,Y,Value,MsgId)
  :  .my_name(miner3) & role(retriever1) & Value >= 3 & Sender >= 0 & not ore(Type,X,Y,Value)
  <- +ore(Type,X,Y,Value);
     .print("Radio received from miner ",Sender+1,": ",ore(Type,X,Y,Value)).
+radio_ore_3(Sender,Type,X,Y,Value,MsgId)
  :  .my_name(miner4) & role(retriever1) & Value >= 3 & Sender >= 0 & not ore(Type,X,Y,Value)
  <- +ore(Type,X,Y,Value);
     .print("Radio received from miner ",Sender+1,": ",ore(Type,X,Y,Value)).
+radio_ore_7(Sender,Type,X,Y,Value,MsgId)
  :  .my_name(miner8) & role(retriever1) & Value >= 3 & Sender >= 0 & not ore(Type,X,Y,Value)
  <- +ore(Type,X,Y,Value);
     .print("Radio received from miner ",Sender+1,": ",ore(Type,X,Y,Value)).
