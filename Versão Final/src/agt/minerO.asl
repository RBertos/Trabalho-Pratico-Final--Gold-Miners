...
// perceived ores are included as self beliefs and shared by radio.
+cell(X,Y,Type,Value) : not ore(Type,X,Y,Value) & .my_name(miner1)
  <- +ore(Type,X,Y,Value);
     broadcastOre(0,Type,X,Y,Value).
+cell(X,Y,Type,Value) : not ore(Type,X,Y,Value) & .my_name(miner2)
  <- +ore(Type,X,Y,Value);
     broadcastOre(1,Type,X,Y,Value).
+cell(X,Y,Type,Value) : not ore(Type,X,Y,Value) & .my_name(miner3)
  <- +ore(Type,X,Y,Value);
     broadcastOre(2,Type,X,Y,Value).
+cell(X,Y,Type,Value) : not ore(Type,X,Y,Value) & .my_name(miner4)
  <- +ore(Type,X,Y,Value);
     broadcastOre(3,Type,X,Y,Value).

+radio_ore_0(Sender,Type,X,Y,Value,MsgId)
  :  .my_name(miner1) & Sender >= 0 & not ore(Type,X,Y,Value)
  <- +ore(Type,X,Y,Value);
     .print("Radio received from miner ",Sender+1,": ",ore(Type,X,Y,Value)).
+radio_ore_1(Sender,Type,X,Y,Value,MsgId)
  :  .my_name(miner2) & Sender >= 0 & not ore(Type,X,Y,Value)
  <- +ore(Type,X,Y,Value);
     .print("Radio received from miner ",Sender+1,": ",ore(Type,X,Y,Value)).
+radio_ore_2(Sender,Type,X,Y,Value,MsgId)
  :  .my_name(miner3) & Sender >= 0 & not ore(Type,X,Y,Value)
  <- +ore(Type,X,Y,Value);
     .print("Radio received from miner ",Sender+1,": ",ore(Type,X,Y,Value)).
+radio_ore_3(Sender,Type,X,Y,Value,MsgId)
  :  .my_name(miner4) & Sender >= 0 & not ore(Type,X,Y,Value)
  <- +ore(Type,X,Y,Value);
     .print("Radio received from miner ",Sender+1,": ",ore(Type,X,Y,Value)).

@pcell[atomic]           // atomic: so as not to handle another event until handle ore is initialised
+ore(Type,X,Y,Value)
  :  not time_over & free & bag_count(C) & bag_capacity(Max) & C < Max
   <- -free;
...
@pcell2[atomic]
+ore(Type,X,Y,Value)
  :  not time_over & not free & bag_count(C) & bag_capacity(Max) & C < Max &
