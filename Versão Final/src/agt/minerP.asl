...
+cell(X,Y,Type,Value)
  :  role(retriever) & miner_id(Id) & Type \== obstacle & not ore(Type,X,Y,Value)
  <- +ore(Type,X,Y,Value);
     broadcastOre(Id,Type,X,Y,Value).
...
+ore(Type,X,Y,Value)
  :  role(retriever) & not time_over & free & bag_count(C) & bag_capacity(Max) & C < Max
   <- -free;
...
@pcell2[atomic]
+ore(Type,X,Y,Value)
  :  role(retriever) & not time_over & not free & bag_count(C) & bag_capacity(Max) & C < Max &
