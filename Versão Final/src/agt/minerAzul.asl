...
+free : not time_over & role(explorer) & search_step(0)
   <- -+search_step(1); !go_near(0,0).
+free : not time_over & role(explorer) & search_step(1)
   <- -+search_step(2); !go_near(34,0).
+free : not time_over & role(explorer) & search_step(2)
   <- -+search_step(3); !go_near(34,7).
+free : not time_over & role(explorer) & search_step(3)
   <- -+search_step(4); !go_near(0,7).
+free : not time_over & role(explorer) & search_step(4)
   <- -+search_step(5); !go_near(0,14).
+free : not time_over & role(explorer) & search_step(5)
   <- -+search_step(6); !go_near(34,14).
+free : not time_over & role(explorer) & search_step(6)
   <- -+search_step(7); !go_near(34,21).
+free : not time_over & role(explorer) & search_step(7)
   <- -+search_step(8); !go_near(0,21).
+free : not time_over & role(explorer) & search_step(8)
   <- -+search_step(9); !go_near(0,28).
+free : not time_over & role(explorer) & search_step(9)
   <- -+search_step(0); !go_near(34,28).
...
+cell(X,Y,Type,Value)
  :  collector & miner_id(Id) & Type \== obstacle & not ore(Type,X,Y,Value)
  <- +ore(Type,X,Y,Value);
     broadcastOre(Id,Type,X,Y,Value).

+radio_ore_0(Sender,Type,X,Y,Value,MsgId)
  :  .my_name(miner1) & collector & Sender >= 0 & not ore(Type,X,Y,Value)
  <- +ore(Type,X,Y,Value);
     .print("Radio received from miner ",Sender+1,": ",ore(Type,X,Y,Value)).
+radio_ore_1(Sender,Type,X,Y,Value,MsgId)
  :  .my_name(miner2) & collector & Sender >= 0 & not ore(Type,X,Y,Value)
  <- +ore(Type,X,Y,Value);
     .print("Radio received from miner ",Sender+1,": ",ore(Type,X,Y,Value)).
+radio_ore_2(Sender,Type,X,Y,Value,MsgId)
  :  .my_name(miner3) & collector & Sender >= 0 & not ore(Type,X,Y,Value)
  <- +ore(Type,X,Y,Value);
     .print("Radio received from miner ",Sender+1,": ",ore(Type,X,Y,Value)).
+radio_ore_3(Sender,Type,X,Y,Value,MsgId)
  :  .my_name(miner4) & collector & Sender >= 0 & not ore(Type,X,Y,Value)
  <- +ore(Type,X,Y,Value);
     .print("Radio received from miner ",Sender+1,": ",ore(Type,X,Y,Value)).
+radio_ore_4(Sender,Type,X,Y,Value,MsgId)
  :  .my_name(miner5) & collector & Sender >= 0 & not ore(Type,X,Y,Value)
  <- +ore(Type,X,Y,Value);
     .print("Radio received from miner ",Sender+1,": ",ore(Type,X,Y,Value)).
+radio_ore_5(Sender,Type,X,Y,Value,MsgId)
  :  .my_name(miner6) & collector & Sender >= 0 & not ore(Type,X,Y,Value)
  <- +ore(Type,X,Y,Value);
     .print("Radio received from miner ",Sender+1,": ",ore(Type,X,Y,Value)).
...
+!after_pick
  :  role(retriever2) & bag_count(C) & C > 0
  <- .print("Blue retriever 2 returns after one ore.");
     !deposit_bag.
+!after_pick
...
+!after_pick
  ...
  <- !!choose_ore.
+!after_pick
  :  role(retriever1) & bag_count(C) & bag_capacity(Max) & C > 0 & C < Max
  <- .print("Blue retriever 1 keeps searching until backpack is full: ",C,"/",Max);
     -+free.
+!after_pick
  :  bag_count(C) & C > 0
 ...
 */
+!choose_ore
  :  role(retriever1) & not ore(_,_,_,_) & bag_count(C) & bag_capacity(Max) & C > 0 & C < Max
  <- -+free.
+!choose_ore
